	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
		 ...
		 for (int i = 0; i < oneHop.size(); i++){
			...
			// Exclude v
			if (v != w){
				System.out.println("Neighbors : " + getNeighbors(w) );
				// twoHop.addAll(getNeighbors(w));
				// Go through the list and add all except v
				List<Integer> interMediate = getNeighbors(w);
				for (int y = 0;  y < interMediate.size(); y++){
					//if (twoHop.size() == 0){
						twoHop.add(interMediate.get(y));
					//}
					//else if (twoHop.indexOf(interMediate.get(y)) == -1) {
					//	twoHop.add(interMediate.get(y));
					//}
				}
			}
		 }
		 //Collections.sort(twoHop);
		 return twoHop;
	}
