...
class MapNode implements Comparable
{
	...
	// get the actual distance between this node and it's neighbor
	public double getDistanceTo(MapNode neighbor)
	{
		for (MapEdge edge : edges) {
			if (neighbor.equals(edge.getOtherNode(this))){
				//System.out.print("Distance between " + this.getLocation() );
				//System.out.println(" and " + neighbor.getLocation() + " is " + edge.getLength());
				return edge.getLength();
			}
		}
		return Double.POSITIVE_INFINITY;
	}
	public double getGeoDistanceTo(GeographicPoint goal)
	{
	...
	public double getTravelTimeTo(MapNode next, boolean rushHour){
		for (MapEdge edge : edges) {
			if (next.equals(edge.getOtherNode(this))){			
				return edge.getTravelTime(rushHour);
			}
		}
	...
	public double getGeoTravelTimeTo(GeographicPoint goal)
	{
		return ( ( this.getLocation().distance(goal) / 100.0 ) * 60 );
	}	
